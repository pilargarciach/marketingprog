...
Con la construcción del diccionario de habilidades, ahora es posible pasar a construir una matriz rectangular término-documento [@Benoit2018]. Esta matriz término-documento está compuesta por un número *n* de filas y un número *c* de columnas, donde las habilidades se disponen en filas y los programas en columnas y los valores en las celdas de tal matriz solo pueden tener dos valores posibles (i.e., 1 para indicar que la habilidad está explícitamente mencionada en el folleto que promociona el programa o cero para indicar lo contrario).
...
# Exploración de la red bipartita

Pasemos ahora a explorar la red bipartita que nos muestra la relación entre habilidades y programas.

```{r, warning=FALSE, message=FALSE}
V(bn2)$shape <- ifelse(V(bn2)$type, "circle", "square")
V(bn2)$label <- ""
plot(bn2, 
     vertex.color = ifelse(V(bn2)$type == FALSE, "#C8102E", "#003087"),
     vertex.size = ifelse(V(bn2)$type, 4, 5), 
     edge.width = 0.3, 
     edge.color = "#FFCD00",
...
Un segundo modelo implica contrastar la hipótesis según la cual las escasas conexiones que observamos en el modelo anterior ocurren debido a un conjunto limitado de habilidades que resultan "más atractivas" para la formación profesional de los estudiantes que se matriculan en las maestrías de marketing. Para probar esta hipótesis, debemos concentrarnos en las competencias como predictoras de la conectividad de la red y observar cuáles de ellas cuentan con un estimado positivo y estadísticamente significativo.
...
De esta exploración, observamos que la habilidad de pensamiento crítico (`b1sociality3`), persuasión (`b1sociality9`), y monitoreo (`b1sociality13`) son las que destacan por su conectividad sistemáticamente superior a lo esperado por azar con la mayoría de folletos.
